...
### 0.2 混合参考库 + 合成参考图标实验（2026-08-13，1000 查询小样本）

**做法**：`mixed/test_pngs` = 原 69 + iconfont 118 = 187 个基础参考；并用新加的
`gentest -augrefs` 模式给每个源图标随机加 1-3 个彩色图形（圆/矩形/三角形）+ 0-2 段
随机文字，每源合成 3 个装饰变体 → 参考池 **748 个**（561 个为合成）。
`gentest -src mixed/test_pngs -out mixed/train_set -n 1000` 合成 1000 查询（覆盖 548 源），
训练 40 epoch / lr=0.002 / 12k 样本。

```
原 test_set（120，同域）:        baseline 88.3%    lean(mixed) 90.8%
iconfont 未见图标（354，跨域）:  baseline 77.4%    lean(mixed) 67.2%   （纯域内训练仅 39.8%）
```

**效果**：跨域 39.8% → 67.2%（+27pp，追平旧架构跨域水平），证明 learned-projection 泛化
差的根因是**训练数据单一**而非架构本身；合成增料参考 + 混合数据是关键修复。同域 90.8%
因小样本训练（1000 查询）低于纯域内模型（92.5%），预计全量 8000 查询训练可继续提升。

### 0.3 结构优化 v2（min+diff 双视角 + token 加宽）→ 全量训练（2026-08-14）

**结构 v2**：
- 每个描述子块喂 **both views**：`[min(q,r) | |q-r|]`（2×49 / 2×288 / 2×16 / 2×192），
  让投影器建模"有向差异"而非只有重叠；
- `nnProjOut 8→12`（`nnPairDim=48`），`nnInput=1138`，参数 **41.7k**（v1=32.6k / 旧 100k）。
- 训练内存修复：`nnSample` 改存 (qIdx,rIdx,y,w)，按 epoch 惰性组装输入向量复用一块缓存
  （95k 样本内存 ~1GB → ~3MB）。

**全量训练**：`mixed/train_set_full`（8000 查询，覆盖全部 748 参考 = 187 原图 + 561 增料合成），
50 epoch / lr=0.002 / 95,659 样本 / ~75min，train@1 峰值 88.9%，权重 `weights_mixed_full.gob`。

```
iconfont 未见图标（354，跨域）:
  baseline adaptive : recall@1 77.4%  @3 82.2%  @5 83.3%
  新 lean v2 全量    : recall@1 77.1%  @3 83.9%  @5 87.3%   ← @1 打平、@3/@5 反超基线
原 test_set（120，同域）:
  baseline adaptive : recall@1 88.3%  @3 94.2%  @5 95.0%
  新 lean v2 全量    : recall@1 91.7%  @3 92.5%  @5 95.0%   （旧架构 96.7%）
```

**收敛路径**（跨域 recall@1）：纯域内训练 39.8% → 混合小样本 67.2% → v2 结构 70.3% →
**全量 77.1%**（追平手工基线）。lean 架构验证成立：41.7k 参数（旧 58%）下跨域达基线
水平且 recall@3/@5 反超；同域 91.7% 距旧架构（96.7%）仍有差距，但旧架构的手工特征
（mask 旋转/颜色直方图等）本就对该域过拟合。后续方向：best/gap 查询统计 token、
模型自举硬负样本、dropout/早停、旋转对齐掩码块。
